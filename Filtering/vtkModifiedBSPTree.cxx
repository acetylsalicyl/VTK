...
//
//----------------------------------------------------------------------------
vtkCxxRevisionMacro(vtkModifiedBSPTree, "$Revision$");
...
void vtkModifiedBSPTree::BuildLocatorInternal()
{
  ...
  for (int i=0; i<3; i++) { // loop over each axis
    for (vtkIdType j=0; j<numCells; j++) { // loop over each cell
...
void vtkModifiedBSPTree::Subdivide(BSPNode *node, Sorted_cell_extents_Lists *lists, vtkDataSet *dataset,
  ...
  // Update depth info
  if (node->depth>MaxDepth) MaxDepth = depth;
  //
  ...
  if ((nCells  > maxCells) && (depth < maxlevel))  {
    ...
    if (found)  {
      ...
      for (vtkIdType i=0; i<nCells; i++) {
        ...
        if    (ext.max < pDiv) left ->Mins[Daxis][Cmin_l[Daxis]++] = ext;
        // min is on right of middle node
        else if (ext.min > pDiv) right->Mins[Daxis][Cmin_r[Daxis]++] = ext;
        // neither - must be one of ours
        else           mid  ->Mins[Daxis][Cmin_m[Daxis]++] = ext;
        //
        // process the MAX-List
        ext = lists->Maxs[Daxis][i];
        // max is on left of middle node
        if    (ext.max < pDiv) left ->Maxs[Daxis][Cmax_l[Daxis]++] = ext;
        // min is on right of middle node
        else if (ext.min > pDiv) right->Maxs[Daxis][Cmax_r[Daxis]++] = ext;
        // neither - must be one of ours
        else           mid  ->Maxs[Daxis][Cmax_m[Daxis]++] = ext;
      }
      ...
      for (Daxis=(node->mAxis+1)%3; Daxis!=node->mAxis; Daxis=(Daxis+1)%3) {
        for (vtkIdType i=0; i<nCells; i++) {
          // process the MIN-List
          cell_extents ext = lists->Mins[Daxis][i];
          if (this->CellBounds[ext.cell_ID][2*node->mAxis+1] < pDiv)  left ->Mins[Daxis][Cmin_l[Daxis]++] = ext;
          else if (this->CellBounds[ext.cell_ID][2*node->mAxis] > pDiv) right->Mins[Daxis][Cmin_r[Daxis]++] = ext;
          else                              mid  ->Mins[Daxis][Cmin_m[Daxis]++] = ext;
          //
          // process the MAX-List
          ext = lists->Maxs[Daxis][i];
          if (this->CellBounds[ext.cell_ID][2*node->mAxis+1] < pDiv)  left ->Maxs[Daxis][Cmax_l[Daxis]++] = ext;
          else if (this->CellBounds[ext.cell_ID][2*node->mAxis] > pDiv) right->Maxs[Daxis][Cmax_r[Daxis]++] = ext;
          else                              mid  ->Maxs[Daxis][Cmax_m[Daxis]++] = ext;
        }
      }
      //
  ...
  for (int i=0; i<6; i++) node->sorted_cell_lists[i] = new vtkIdType[nCells];
  //
  for (int i=0; i<3; i++) {
...
bool BSPNode::RayMinMaxT(const double origin[3], const double dir[3], double &rTmin, double &rTmax) const {
  ...
  if (dir[0] < -Epsilon_) {                // ray travelling in -x direction
    ...
    if (tT < rTmin) return (false);        // ray already left of box. Can't hit
    if (tT <= rTmax) rTmax = tT;           // update new tmax
    tT = (bounds[1] - origin[0]) / dir[0]; // distance to right edge
    if (tT >= rTmin) {                     // can't see this ever happening
      if (tT > rTmax) return false;        // clip start of ray to right edge
      rTmin = tT;
    }
  }
  else if (dir[0] > Epsilon_) {
    tT = (bounds[1] - origin[0]) / dir[0];
    if (tT < rTmin) return (false);
    if (tT <= rTmax) rTmax = tT;
    tT = (bounds[0] - origin[0]) / dir[0];
    if (tT >= rTmin) {
      if (tT > rTmax) return (false);
      rTmin = tT;
    }
  }
  else if (origin[0] < bounds[0] || origin[0] > bounds[1]) return (false);
  // Y-Axis
  if (dir[1] < -Epsilon_) {
  ...
  else if (origin[1] < bounds[2] || origin[1] > bounds[3]) return (false);
  // Z-Axis
  if (dir[2] < -Epsilon_) {
  ...
  else if (origin[2] < bounds[4] || origin[2] > bounds[5]) return (false);
